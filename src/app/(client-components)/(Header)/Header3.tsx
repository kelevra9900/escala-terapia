...
const Header3: FC<Header3Props> = ({className = ''}) => {
	...
	const renderButtonOpenHeroSearch = () => {
		return (
			<div
				className={`relative flex w-full items-center justify-between rounded-full border border-neutral-200 shadow transition-all hover:shadow-md dark:border-neutral-600 ${showHeroSearch
	...
	return (
		<>
			<div
				className={`nc-Header nc-Header-3 fixed inset-0 top-0 z-40 bg-black/30 transition-opacity will-change-[opacity] dark:bg-black/50 ${showHeroSearch ? 'visible' : 'pointer-events-none invisible opacity-0'
					}`}
			></div>
			{showHeroSearch && <div id="nc-Header-3-anchor"></div>}
			<header ref={headerInnerRef} className={`sticky top-0 z-40 ${className}`}>
				<div
					className={`absolute inset-x-0 top-0 h-full bg-white transition-transform will-change-[transform,opacity] dark:bg-neutral-900 ${showHeroSearch ? 'duration-75' : ''} ${showHeroSearch
